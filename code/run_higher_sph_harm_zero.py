...
# Import functions
import numpy as np
import eigencurves_priors
import os

from importlib import import_module


def run_lc_fit(datadict,planetparams,norder=3,lcName='lcname',saveDir='saveDir',firstfits='firstfits',afew=10,burnin=100,nsteps=1000,plot=False,strict=True,nonegs=True):
    """
    ...
    if os.path.exists(firstfits) == False:
        assert os.path.exists(firstfits),"Error: could not locate directory {}".format(firstfits)

    if os.path.exists(saveDir) == False:
        os.makedirs(saveDir)
    if isinstance(norder,list):
        if isinstance(afew,int):
            if afew>=16:
                outputNPZ = '{}/spherearray_deg_list_eigenfit.npz'.format(saveDir,norder)
            else:
                outputNPZ = '{}/spherearray_deg_list_eigen_{}.npz'.format(saveDir,norder,afew)
        else:
            outputNPZ = '{}/spherearray_deg_list.npz'.format(saveDir,norder)
    else:
        if isinstance(afew,int):
            if afew>=16:
    ...
    if os.path.exists(outputNPZ):
        ...
        spherearray = eigencurves_priors.eigencurves(datadict,planetparams,firstfits,plot=plot,degree=norder,afew=afew,burnin=burnin,nsteps=nsteps,strict=strict,nonegs=nonegs)#_starry
